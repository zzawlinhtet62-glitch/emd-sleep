...
不是一篇有插圖的文章。三個分頁都是可操作的演算法：

| 分頁 | 你能做什麼 | 看到什麼 |
|---|---|---|
| 一、篩選演算法 | 按「下一步」逐格走過篩選過程；拖滑桿選取樣點 | 極值 → 上包絡 → 下包絡 → 中線 → `h = 訊號 − 中線` → IMF 兩條件檢查；以及一張可自行驗算的即時數值表 |
| 二、五個睡眠期 | 切換 Wake / N1 / N2 / N3 / REM；拖動雜訊滑桿 | 每一期的完整 EMD 分解，每條 IMF 標註主頻、峰值振幅、腦波頻帶；拉大雜訊會看到雜訊集中到 IMF1 |
| 三、傅立葉 vs 希爾伯特 | 移動紡錘波的出現時間與頻率 | 左邊傅立葉振幅頻譜幾乎不動，右邊希爾伯特譜的亮塊跟著時間走 |

第三個分頁是整個網站的論點：傅立葉頻譜只回答「有哪些頻率」，
...
`node test/test.js` 的實測結果（13 項全部通過）：
