...
- 動態只有三處：首頁的記錄紙持續捲動，以及第 03 章的「自動播放」與第 06 章的「自動掃描」。
  捲動的訊號是一段 81.92 秒的週期（慢波振幅起伏、中間夾三個紡錘波與三個 K 複合波），
  所有連續成分都是 1/週期的整數倍、雜訊與事件都放在同一個週期內再平鋪，
  因此嚴格週期、可無縫循環；接縫步幅落在各層正常取樣間距的 95 百分位內，肉眼看不出。
  每一列以該列的 97 百分位縮放並裁切，否則單一個 70 µV 的 K 複合波會把其他時候的活動壓成直線
  （實測 IMF1 被壓 4.8 倍、IMF2 被壓 9.7 倍）。
  `prefers-reduced-motion` 下捲動靜止，切換章節或分頁隱藏時自動停止。手機版可用。
- 語言完全不記憶：每次載入一律中文，按鈕只切換當下這一次。要連到英文版可用 `?lang=en`。
  （先前存過的偏好會在載入時清除，不會再冒出來。）
