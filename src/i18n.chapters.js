...
(function (global) {
  ...
  const EXTRA = {
    zh: {
      ...
      tab_mix: '07 模態混合與集成經驗模態分解',
      tab_paper: '08 論文案例',

      /* ---- 01 局部極值 ---- */
      exTitle: '一切從「比較左右鄰居」開始',
      exIntro: '經驗模態分解沒有基底函數，它唯一的輸入是極值點的位置。判定方法簡單到近乎粗暴：' +
      ...
      exNote: '拉高第二個成分的頻率，極值點會迅速變多，而過零點不會同步增加——' +
              '兩者的差距正是本質模態函數條件一在量的東西：一條曲線是不是「每個起伏都對稱地繞著零」。' +
      ...
      envNote: '三種規則在中段幾乎完全重合，只在兩端分岔——這就是端點效應：誤差從邊界產生，' +
               ...
               '線性外推讓模態3之後的殘餘能量從 10.164% 降到 0.001%。規則要挑，得看訊號。',

      /* ---- 04 訊號實驗室 ---- */
      labTitle: '自己組一段訊號，看它怎麼被拆開',
      labIntro: '三個正弦成分，頻率與振幅都可以拉。拉完馬上做完整的經驗模態分解。' +
                '這裡沒有預先算好的答案——你改一個數字，整串本質模態函數就重算一次。',
      labComp: '成分 %1',
      ...
      labPresetSlow: '慢波加快波',
      labReconTitle: '重建檢查',
      labRecon: '所有本質模態函數加上殘量，與原訊號的最大誤差：<b>%1</b>（%2 個取樣點）',
      labReconNote: '經驗模態分解是完備的分解：不管你把參數調成什麼樣，加回去一定等於原訊號。' +
                    '這個誤差只會是浮點數的捨入等級。',
      labNote: '把兩個頻率拉到很接近（例如 4 Hz 與 5 Hz），經驗模態分解會把它們併成同一條本質模態函數——' +
      ...
      mixIntro: '一條 1 Hz 的慢波，加上只在幾個瞬間出現的快速爆發。' +
                '這種「間歇性」正是經驗模態分解最大的弱點：在有爆發的區段，篩選看到的是快波；' +
                '在沒有爆發的區段，它看到的是慢波。於是同一條模態1裡混進了兩種完全不同的頻率。',
      mixBursts: '爆發次數',
      mixFreq: '爆發頻率',
      mixMethod: '分解方法',
      mixMethodEmd: '經驗模態分解',
      mixMethodEemd: '集成經驗模態分解',
      mixEns: '集成數',
      mixNoiseAmp: '加入雜訊（訊號標準差的倍數）',
      mixTrue: '真實的爆發成分',
      mixCorrTitle: '模態1與真實爆發成分的相關係數',
      mixCorrEmd: '經驗模態分解：<b>%1</b>',
      mixCorrEemd: '集成經驗模態分解（%1 組集成，雜訊 %2 σ）：<b class="hi">%3</b>',
      mixExplain: '集成經驗模態分解的做法違反直覺：既然問題出在訊號在某些區段「沒有東西可以篩」，' +
                  '那就補上白雜訊，讓每個區段都有均勻分布的尺度可用。' +
                  '對很多組「原訊號 + 不同雜訊」各做一次經驗模態分解，再把對應的本質模態函數平均——' +
                  '雜訊互相抵消，真正的成分留下來［2］。Wu 與 Huang 特別指出雜訊必須是有限振幅、' +
                  '而非無限小，才能迫使集成窮盡所有可能解。',
      mixNote: '代價是：集成後的本質模態函數不再完全正交，計算量乘上集成數，' +
      ...
      paperMethodEvo: '方法演化：經驗模態分解 (1998) → 集成經驗模態分解 (2009) → 自適應雜訊完全集成經驗模態分解 (2016)。' +
                      '三者的差別都在同一件事上：如何處理模態混合。',
      papers: [
        {
          ref: 4,
          head: '集成經驗模態分解 + XGBoost 的單導程自動睡眠分期',
          task: '用一條腦波通道判讀整夜睡眠分期，取代人工判讀。',
          method: '把腦波以集成經驗模態分解分解，從原始訊號與各條本質模態函數同時抽取統計特徵、時域特徵與' +
                  '非線性動力學特徵，交給 XGBoost，五折交叉驗證。',
          result: 'Sleep-EDF 四分類 93.1%、五分類 91.9%；DREAMS 86.4% / 83.4%；SHHS 87.5% / 85.8%。',
          point: '關鍵結論不是準確率本身，而是：本質模態函數導出的特徵確實提供了原始訊號以外的額外資訊。'
        },
        {
          ref: 7,
          head: '只用清醒時 5 分鐘的呼吸訊號篩檢睡眠呼吸中止',
          task: '常規阻塞型睡眠呼吸中止診斷需要整夜多導記錄；這篇問的是能不能只用清醒時段的短訊號篩檢。',
          method: '取清醒時 5 分鐘的口鼻氣道壓力訊號，以希爾伯特–黃轉換抽取本質振盪模態，' +
                  '比較第一與第二模態的頻率分布。',
          result: '41 名受試者（非阻塞型睡眠呼吸中止 20 人、阻塞型睡眠呼吸中止 21 人），兩組的模態頻率分布有顯著差異；' +
                  '所建指標在 95% 特異度下達 81.0% 敏感度。',
          point: '呼吸訊號本質上是非穩態的——這正是希爾伯特–黃轉換相對於傅立葉的主場。'
        },
        {
          ref: 2,
          head: '集成經驗模態分解：用雜訊解決模態混合',
          task: '直接針對經驗模態分解的最大弱點：間歇性造成的模態混合。',
          method: '對「原訊號加白雜訊」的多組集成樣本各做一次完整篩選，再把對應的本質模態函數平均。',
        ...
        {
          ref: 6,
          head: '自適應雜訊完全集成經驗模態分解 ＋裝袋法的電腦輔助睡眠分期',
          task: '延續 [5] 的路線，把分解方法換成自適應雜訊完全集成經驗模態分解。',
          method: '完全集成經驗模態分解搭配自適應雜訊，再以自助聚合法（裝袋法）分類。',
          result: '與 [1][2] 並列，構成方法的演化軌跡：經驗模態分解 → 集成經驗模態分解 → 自適應雜訊完全集成經驗模態分解。',
          point: '自適應雜訊完全集成經驗模態分解在每一階分解後才加入自適應雜訊，解決集成經驗模態分解殘留雜訊與各集成本質模態函數數不一致的問題。'
        },
        {
          ref: 8,
          head: '全希爾伯特譜分析用於全身麻醉下的腦波',
          task: '分析麻醉深度與疼痛—鎮痛平衡的腦波調變模式。',
          method: '用全希爾伯特譜分析，同時辨識振幅調變與頻率調變。',
          result: '黃鍔院士與台灣團隊的近期合作成果。',
          point: '論文指出傳統頻譜方法必須事先切定頻帶再帶通濾波，' +
                 '難以偵測不同振盪成分之間的非線性交互作用——這是改用全希爾伯特譜分析的理由。'
        }
      ],
      paperTable: [
        ['Sleep-EDF', '93.1%', '91.9%'],
      ...
      verdictNoWhy: '第 %1 輪的 h 還不是本質模態函數：%2。把 h 當作新訊號，回到步驟 1 再篩一次。',
