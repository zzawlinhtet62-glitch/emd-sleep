  ...
  const EXTRA = {
    zh: {
      ...
      exNote: '拉高第二個成分的頻率，極值點會迅速變多，而過零點不會同步增加——' +
              '兩者的差距正是本徵模態函數條件一在量的東西：一條曲線是不是「每個起伏都對稱地繞著零」。' +
      ...
      labIntro: '三個正弦成分，頻率與振幅都可以拉。拉完馬上做完整的經驗模態分解。' +
                '這裡沒有預先算好的答案——你改一個數字，整串本徵模態函數就重算一次。',
      labComp: '成分 %1',
      ...
      labPresetSlow: '慢波加快波',
      labReconTitle: '重建檢查',
      labRecon: '所有本徵模態函數加上殘量，與原訊號的最大誤差：<b>%1</b>（%2 個取樣點）',
      labReconNote: '經驗模態分解是完備的分解：不管你把參數調成什麼樣，加回去一定等於原訊號。' +
                    '這個誤差只會是浮點數的捨入等級。',
      labNote: '把兩個頻率拉到很接近（例如 4 Hz 與 5 Hz），經驗模態分解會把它們併成同一條本徵模態函數——' +
      ...
      mixExplain: '集成經驗模態分解的做法違反直覺：既然問題出在訊號在某些區段「沒有東西可以篩」，' +
                  ...
                  '對很多組「原訊號 + 不同雜訊」各做一次經驗模態分解，再把對應的本徵模態函數平均——' +
                  '雜訊互相抵消，真正的成分留下來［2］。Wu 與 Huang 特別指出雜訊必須是有限振幅、' +
                  '而非無限小，才能迫使集成窮盡所有可能解。',
      mixNote: '代價是：集成後的本徵模態函數不再完全正交，計算量乘上集成數，' +
      ...
      papers: [
        {
          ...
          method: '把腦波以集成經驗模態分解分解，從原始訊號與各條本徵模態函數同時抽取統計特徵、時域特徵與' +
                  '非線性動力學特徵，交給 XGBoost，五折交叉驗證。',
          result: 'Sleep-EDF 四分類 93.1%、五分類 91.9%；DREAMS 86.4% / 83.4%；SHHS 87.5% / 85.8%。',
          point: '關鍵結論不是準確率本身，而是：本徵模態函數導出的特徵確實提供了原始訊號以外的額外資訊。'
        },
        {
          ref: 7,
        ...
        },
        {
          ...
          method: '對「原訊號加白雜訊」的多組集成樣本各做一次完整篩選，再把對應的本徵模態函數平均。',
        ...
        },
        {
          ...
          point: '自適應雜訊完全集成經驗模態分解在每一階分解後才加入自適應雜訊，解決集成經驗模態分解殘留雜訊與各集成本徵模態函數數不一致的問題。'
        },
        {
          ref: 8,
      ...
      verdictNoWhy: '第 %1 輪的 h 還不是本徵模態函數：%2。把 h 當作新訊號，回到步驟 1 再篩一次。',
