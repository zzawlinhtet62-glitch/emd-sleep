  ...
  const STR = {
    zh: {
      ...
      p1Intro: '合成訊號由三個正弦波疊成。按「下一步」走過篩選的每個動作：找極值、連上下包絡、取中線、相減、檢查。' +
               '如果結果不符合本質模態函數的兩個條件，就把相減後的 h 當成新訊號，再篩一次。',
      p1Next: '下一步',
      p1Reset: '重新開始',
      p1Round: '第 %1 輪',
      p1Step: '步驟 %1 / 6',
      ...
      verdictNo: 'h 還不是本質模態函數。把 h 當作新訊號，回到步驟 1 再篩一次。',
      ...
      p2Intro: '依睡眠醫學的描述合成五段 10.24 秒的腦波，然後對每一段做完整的經驗模態分解。' +
               '每條本質模態函數左邊標出主頻、峰值振幅與對應的腦波頻帶。主頻取的是「振幅加權的平均瞬時頻率」' +
               '（由希爾伯特轉換算出），不是過零率——過零率對間歇性的本質模態函數會嚴重高估，' +
      ...
      why1: '人工判讀一夜的多導睡眠記錄要好幾個小時，而且判讀者之間的一致性有限。' +
            '把單導程腦波用集成經驗模態分解分解後，從原始訊號與各本質模態函數同時抽取統計、時域與非線性動力學特徵，' +
            'Sleep-EDF 上的四分類準確率可達 93.1%、五分類 91.9%；DREAMS 與 SHHS 資料庫也在 83–88% 之間［4］。' +
            '關鍵結論是：本質模態函數導出的特徵確實提供了原始訊號以外的額外資訊。',
      why2Title: '偽跡移除',
      why2: '眼動、肌電、電極接觸不良都會汙染腦波。傳統做法是帶通濾波，但濾波會把落在同一頻帶的真實腦波一起削掉。' +
            '經驗模態分解的分層是資料自適應的：偽跡通常集中在最前面一兩層本質模態函數，把那幾層丟掉再重建，' +
      ...
      lim1Title: '模態混合',
      lim1: '同一條本質模態函數裡出現差距很大的頻率，或者同一個頻率成分散落在相鄰的幾條本質模態函數。' +
      ...
      lim3: '本質模態函數之間只是「近似正交」，沒有理論保證；整個方法目前也還沒有完整的數學收斂證明，' +
      ...
      implList: [
        ...
        ['模態條件檢查', '本質模態函數兩條件的檢查，回傳極值數、過零數與包絡中線比值。'],
        ['反覆篩選', '反覆篩選，標準差準則 < 0.2 停止，上限 12 次。'],
        ['經驗模態分解', '反覆抽取本質模態函數並從殘量中扣除，直到極值不足或能量過小。'],
        ['快速傅立葉轉換／振幅頻譜', '疊代式基-2 原地快速傅立葉轉換，以及單邊振幅頻譜。'],
        ['希爾伯特轉換', '以快速傅立葉轉換建構解析訊號，相位差解纏繞後換算成瞬時頻率。'],
        ['集成經驗模態分解', '集成經驗模態分解：對多組「原訊號 + 白雜訊」各做一次經驗模態分解，再平均對應的本質模態函數。'],
        ['平均瞬時頻率', '振幅加權的平均瞬時頻率，用來標註每條本質模態函數的主頻。']
      ],
      implTests: '演算法先用 Node 測過才接上畫面。測試內容包括：模態1與 13 Hz 成分的相關係數 0.999938；' +
                 '所有本質模態函數加殘量重建原訊號的最大誤差 1.78 × 10⁻¹⁵；' +
      ...
      refNotes: [
        ...
        '以集成經驗模態分解分解腦波，從原始訊號與本質模態函數同時抽取統計、時域與非線性動力學特徵，用 XGBoost 五折交叉驗證。Sleep-EDF 四分類 93.1%、五分類 91.9%；DREAMS 86.4% / 83.4%；SHHS 87.5% / 85.8%。結論：本質模態函數導出的特徵確實提供額外資訊。',
        '',
