...
(function (global) {
  ...
  const STR = {
    zh: {
      ...
      heroLede: '經驗模態分解不預設任何基底函數。它只問訊號自己：局部的起伏有多快？' +
                '這個網站不是在講解演算法，而是把演算法交到你手上——每一條曲線都是你按下按鈕的當下，在瀏覽器裡算出來的。',
      stripLeft: '合成睡眠腦波：慢波振幅起伏，中間夾著紡錘波與 K 複合波 · fs = 100 Hz · 視窗 10.24 s',
      stripRight: '原始訊號與其本徵模態函數（由上而下，頻率遞減）',
      ...
      p1Intro: '合成訊號由三個正弦波疊成。按「下一步」走過篩選的每個動作：找極值、連上下包絡、取中線、相減、檢查。' +
               '如果結果不符合本徵模態函數的兩個條件，就把相減後的 h 當成新訊號，再篩一次。',
      p1Next: '下一步',
      p1Reset: '重新開始',
      p1Round: '第 %1 輪',
      p1Step: '步驟 %1 / 6',
      ...
      c2Val: '%1%',
      verdictOk: 'h 是一個本徵模態函數。這一層抽出來了，剩下的殘量會回到步驟 0 繼續分解。',
      verdictNo: 'h 還不是本徵模態函數。把 h 當作新訊號，回到步驟 1 再篩一次。',
      ...
      /* ---- tab 2 ---- */
      p2Title: '五個睡眠期分解出什麼',
      p2Intro: '依睡眠醫學的描述合成五段 10.24 秒的腦波，然後對每一段做完整的經驗模態分解。' +
               '每條本徵模態函數左邊標出主頻、峰值振幅與對應的腦波頻帶。主頻取的是「振幅加權的平均瞬時頻率」' +
               '（由希爾伯特轉換算出），不是過零率——過零率對間歇性的本徵模態函數會嚴重高估，' +
               '第 07 章有一個 12 Hz 的爆發被過零率讀成 25 Hz 的例子。',
      p2Noise: '加入雜訊',
      p2NoiseUnit: 'µV',
      p2Peak: '峰值',
      imfLabel: '模態',
      stageNames: { wake: '清醒', n1: 'N1 淺睡', n2: 'N2', n3: 'N3 深睡', rem: '快速動眼期' },
      stageDesc: {
      ...
      p2NoiseNote: '把雜訊滑桿從 0 拉開：模態1的主頻會從 %1 Hz 跳到 %2 Hz，能量也跟著上升。' +
                   '雜訊被集中到最高頻的那一層，底下幾層幾乎沒受影響——' +
                   '這就是經驗模態分解可以用來移除偽跡的原因：丟掉模態1，再把剩下的加回去。',
      p2NoiseNoteZero: '目前沒有加雜訊。模態1的主頻是 %1 Hz。把滑桿拉開看看它怎麼變。',
      ...
      why1: '人工判讀一夜的多導睡眠記錄要好幾個小時，而且判讀者之間的一致性有限。' +
            '把單導程腦波用集成經驗模態分解分解後，從原始訊號與各本徵模態函數同時抽取統計、時域與非線性動力學特徵，' +
            'Sleep-EDF 上的四分類準確率可達 93.1%、五分類 91.9%；DREAMS 與 SHHS 資料庫也在 83–88% 之間［4］。' +
            '關鍵結論是：本徵模態函數導出的特徵確實提供了原始訊號以外的額外資訊。',
      why2Title: '偽跡移除',
      why2: '眼動、肌電、電極接觸不良都會汙染腦波。傳統做法是帶通濾波，但濾波會把落在同一頻帶的真實腦波一起削掉。' +
            '經驗模態分解的分層是資料自適應的：偽跡通常集中在最前面一兩層本徵模態函數，把那幾層丟掉再重建，' +
            '其他頻率成分保持完整。分頁二的雜訊滑桿示範的就是這件事。',
      why3Title: '呼吸與睡眠呼吸中止',
      why3: '常規阻塞型睡眠呼吸中止診斷需要整夜的多導記錄。有研究改用清醒時段僅 5 分鐘的口鼻氣道壓力訊號，' +
            '以希爾伯特–黃轉換抽取本質振盪模態：41 名受試者中，第一與第二模態的頻率分布在阻塞型睡眠呼吸中止與非阻塞型睡眠呼吸中止兩組間有顯著差異，' +
            '所建指標在 95% 特異度下達到 81.0% 敏感度［7］。呼吸訊號本質上是非穩態的，' +
            '這正是經驗模態分解的主場。',
      why4Title: '麻醉深度與疼痛',
      why4: '黃鍔院士與台灣團隊近期把全希爾伯特譜分析用在全身麻醉下的腦波調變模式。' +
            '他們指出傳統頻譜方法必須事先切定頻帶再帶通濾波，難以偵測不同振盪成分之間的非線性交互作用［8］。',

      /* ---- limits ---- */
      limEyebrow: '限制',
      limTitle: '經驗模態分解的已知弱點',
      limIntro: '一個方法值不值得用，要看它承不承認自己什麼時候會失敗。經驗模態分解有兩個眾所周知的問題。',
      lim1Title: '模態混合',
      lim1: '同一條本徵模態函數裡出現差距很大的頻率，或者同一個頻率成分散落在相鄰的幾條本徵模態函數。' +
            '成因是訊號裡的間歇事件：當一個短暫的高頻爆發只出現在局部，篩選過程在有它和沒它的區段會做出不同的判斷。' +
            '分頁二的 N2 就看得到——紡錘波只在兩個瞬間出現，所以模態1在有紡錘波的地方跟著 13 Hz 跑，' +
            '其他地方跟著背景跑。解法是集成經驗模態分解：對「原訊號加白雜訊」的多組樣本各做一次分解再平均，' +
      ...
      lim3: '本徵模態函數之間只是「近似正交」，沒有理論保證；整個方法目前也還沒有完整的數學收斂證明，' +
            '停止準則（標準差門檻、篩選次數上限）本質上是經驗性的。換一組參數，分解結果會有些許不同。',
      ...
      implIntro: '所有數值都是在你的瀏覽器裡即時算出來的，沒有預先產生的圖，也沒有用任何函式庫。' +
                 '以下函式全部手寫：',
      implList: [
        ['三次樣條內插', '自然三次樣條。用追趕法解三對角系統求各節點的二階導數，再逐段求值。'],
        ['局部極值搜尋', '逐點與左右鄰居比較找局部極值，平台區段收斂成單一點。'],
        ['包絡線建構', '把極值點樣條連成包絡線，兩端以最外側兩點線性外推並夾限。'],
        ['單次篩選', '一次篩選：算上下包絡、取中線、相減。'],
        ['模態條件檢查', '本徵模態函數兩條件的檢查，回傳極值數、過零數與包絡中線比值。'],
        ['反覆篩選', '反覆篩選，標準差準則 < 0.2 停止，上限 12 次。'],
        ['經驗模態分解', '反覆抽取本徵模態函數並從殘量中扣除，直到極值不足或能量過小。'],
        ['快速傅立葉轉換／振幅頻譜', '疊代式基-2 原地快速傅立葉轉換，以及單邊振幅頻譜。'],
        ['希爾伯特轉換', '以快速傅立葉轉換建構解析訊號，相位差解纏繞後換算成瞬時頻率。'],
        ['集成經驗模態分解', '集成經驗模態分解：對多組「原訊號 + 白雜訊」各做一次經驗模態分解，再平均對應的本徵模態函數。'],
        ['平均瞬時頻率', '振幅加權的平均瞬時頻率，用來標註每條本徵模態函數的主頻。']
      ],
      implTests: '演算法先用 Node 測過才接上畫面。測試內容包括：模態1與 13 Hz 成分的相關係數 0.999938；' +
                 '所有本徵模態函數加殘量重建原訊號的最大誤差 1.78 × 10⁻¹⁵；' +
                 '模態1的平均瞬時頻率 13.0000 Hz、平均瞬時振幅 3.0000；' +
      ...
      refNotes: [
        ...
        '用巢狀經驗模態分解與希爾伯特–黃轉換，同時辨識振幅調變與頻率調變。',
        '以集成經驗模態分解分解腦波，從原始訊號與本徵模態函數同時抽取統計、時域與非線性動力學特徵，用 XGBoost 五折交叉驗證。Sleep-EDF 四分類 93.1%、五分類 91.9%；DREAMS 86.4% / 83.4%；SHHS 87.5% / 85.8%。結論：本徵模態函數導出的特徵確實提供額外資訊。',
        '',
        '與 [1][2] 並列可構成方法演化軌跡：經驗模態分解 (1998) → 集成經驗模態分解 (2009) → 自適應雜訊完全集成經驗模態分解 (2016)。',
        '常規診斷需整夜記錄；本研究改用清醒時段僅 5 分鐘的口鼻氣道壓力訊號。41 名受試者（非阻塞型睡眠呼吸中止 20 人、阻塞型睡眠呼吸中止 21 人），以希爾伯特–黃轉換抽取本質振盪模態，第一與第二模態的頻率分布在兩組間有顯著差異，所建指標在 95% 特異度下達 81.0% 敏感度。',
        '黃鍔院士與台灣團隊的近期合作。指出傳統頻譜方法需事先切定頻帶再帶通濾波，難以偵測不同振盪成分間的非線性交互作用，因而改用全希爾伯特譜分析。'
      ],
      refSupp: '在約 120 Hz 取樣率下：模態1約 20–60 Hz（快速振盪強度）、模態3約 2–4 Hz、模態4約 1–2 Hz、模態5約 0.2–1 Hz。模態3+4+5 的合併振幅（0.2–4 Hz）可作為慢波活動強度的量測。',
    ...
    en: {
      ...
      stripLeft: 'Synthetic sleep EEG: slow waves waxing and waning, with spindles and K-complexes passing through · fs = 100 Hz · 10.24 s window',
